...
void CON_AlphaGL(SDL_Surface *s, int alpha) {
	...
	case 4: {
			...
			Uint8 *pixel8;
			if((numpixels & 0x7) == 0)
				for(pixel8 = pix + 3; pixel8 < last; pixel8 += 32)
					*pixel8 = *(pixel8 + 4) = *(pixel8 + 8) = *(pixel8 + 12) = *(pixel8 + 16) = *(pixel8 + 20) = *(pixel8 + 24) = *(pixel8 + 28) = val;
			else
				for(pixel8 = pix + 3; pixel8 < last; pixel8 += 4)
					*pixel8 = val;
			break;
		}
	default:
