/*
** graphic_texture.c for battletrantor in /home/rannou_s/Work/graoom/client/graphic
** 
** Made by sebastien rannou
** Login   <rannou_s@epitech.net>
** 
** Started on  Thu Jul 23 19:55:08 2009 sebastien rannou
** Last update Mon Oct  5 17:31:35 2009 sebastien rannou
*/
...
list_t *
global_textures = NULL;

/**!
 * Get a texture surface from its name
 */
...
{
  list_t		*current;
  internal_texture_t	*elem;

  for (current = global_textures; current != NULL; current = current->li_next)
    {
      elem = current->data;
  ...
  ERR_RAISE(EC_GL_GET_TEXTURE, name);
  return NULL;
}

/**!
 * Get a texture ID from its name
 */
...
#if 0 /* do some search here ;) */

  gluBuild2DMipmaps(GL_TEXTURE_2D, 4, converted->w,
 		    converted->h, GL_RGBA,GL_UNSIGNED_BYTE,
		    converted->pixels);
...
graphic_load_texture(char *path, char *name)
{
  ...
  LOG(GRAPHIC_LOADING_TEXTURE_OK, path);
  return (SUCCESS);
}
